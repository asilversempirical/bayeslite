...
def failprob_threshold(observed, ns, threshold):
    ...
    # mlx is the integral over the unit interval of (q**ns)*PB(q,q-1), which is
    # the given formula
...
def test_failprob_threshold():
    ...
    def sample(n):
        return [random.gauss(0, 1) for _ in itertools.repeat(None, n)]
    ...
    samples = [all(v < thresh for v in sample(test_sample_size))
               for _ in itertools.repeat(None, int(100/target_prob))]
...
def compute_sufficiently_stringent_threshold(generator, ns, threshold):
    ...
    observed = []
    while True:
        observed.extend(generator() for _ in itertools.repeat(None, batchsize))
